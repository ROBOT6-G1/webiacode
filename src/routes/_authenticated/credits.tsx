...
function CreditsPage() {
  ...
  const amount =
    plan === "pro" ? 5000 : plan === "ai" ? 10000 : (credits / CREDIT_STEP) * PRICE_PER_STEP;

  const creditsAwarded = plan === "pro" ? 15 : plan === "ai" ? 0 : credits;
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <button
          ...
          className={`rounded-2xl border p-6 text-left transition-colors ${plan === "credits" ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/50"}`}
        >
          <h3 className="text-lg font-semibold">Pack Crédits</h3>
          <p className="text-sm text-muted-foreground mt-1">
            {CREDIT_STEP} crédits = {PRICE_PER_STEP.toLocaleString()} Ar · palier de {CREDIT_STEP}
          </p>
        </button>
        <button
          onClick={() => setPlan("pro")}
          className={`rounded-2xl border p-6 text-left transition-colors ${plan === "pro" ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/50"}`}
        >
          <h3 className="text-lg font-semibold">Plan PRO — 5 000 Ar/mois</h3>
          <p className="text-sm text-muted-foreground mt-1">
            +15 crédits bonus · sans logo · domaine perso · Firebase illimité (30 jours)
          </p>
        </button>
        <button
          onClick={() => setPlan("ai")}
          className={`rounded-2xl border p-6 text-left transition-colors ${plan === "ai" ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/50"}`}
        >
          <h3 className="text-lg font-semibold">Abonnement IA — 10 000 Ar/mois</h3>
          <p className="text-sm text-muted-foreground mt-1">
            Utilise TA clé IA (30+ fournisseurs) · zéro crédit consommé pendant 30 jours
          </p>
        </button>
      </div>
      ...
      {plan === "credits" && (
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Exemple : {CREDIT_STEP} crédits = {PRICE_PER_STEP.toLocaleString()} Ar ·{" "}
            {CREDIT_STEP * 2} crédits = {(PRICE_PER_STEP * 2).toLocaleString()} Ar ·{" "}
            {CREDIT_STEP * 4} crédits = {(PRICE_PER_STEP * 4).toLocaleString()} Ar.
          </p>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-primary/30 bg-primary/5 p-6 space-y-3">
        ...
        <div className="text-sm space-y-1 pt-3 border-t border-border">
          <p>
            <strong>Numéro Orange Money :</strong> 0323911654
          </p>
          <p>
            <strong>Nom :</strong> RAVELOMANANTSOA URMIN
          </p>
          <p>
            <strong>Montant :</strong> {amount.toLocaleString()} Ar
          </p>
          {plan === "ai" && (
            <p className="text-xs text-muted-foreground pt-1">
              Après validation, votre clé IA personnelle sera active 30 jours sans consommer de
              crédits DEVWEBIA.
            </p>
          )}
        </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6 space-y-4">
        <div>
          <Label htmlFor="ref">Référence de la transaction Orange Money *</Label>
          <Input
            id="ref"
            value={ref}
            onChange={(e) => setRef(e.target.value)}
            placeholder="Ex: MP2401..."
            className="mt-1"
          />
        </div>
        <div>
            ...
            >
              <div>
                <p className="font-medium">
                  {p.amount_ar.toLocaleString()} Ar —{" "}
                  {p.kind === "ai_sub" ? "Abo IA (30j)" : `${p.credits} crédits`}
                </p>
                <p className="text-xs text-muted-foreground">
                  {new Date(p.created_at).toLocaleString("fr-FR")} · Réf {p.reference}
                </p>
              </div>
              {p.status === "validated" && (
